...
def generate_london_articles():
    ...
    trends = llm_helper.fetch_live_trends(["london", "londonsocialclub", "ukmusic"])
    
    for topic in LONDON_TOPICS:
        paras = llm_helper.call_gemini_synthesis("London", topic['title'], trends) or topic['paras']
...
<body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 800px; margin: 40px auto; padding: 20px; line-height: 1.6; color: #111;">
    ...
    llm_helper.generate_dynamic_trend_article("London", LONDON_DIR, trends, "london")
